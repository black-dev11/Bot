...
function loadLocalFiles() {
  ...
  // Check for shehbaz.html
  if (fs.existsSync('./lib/shehbaz.html')) {
    console.log("✅ shehbaz.html found in lib folder");
  } else {
    console.log("⚠️ shehbaz.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("Shehbaz-MD!")) {
    console.log("❌ Invalid SHEHBAZ-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ SHEHBAZ-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'shehbaz.html');
  if (fs.existsSync(htmlPath)) {
    ...
    res.send(`
      ...
      <html>
      <head>
        <title>SHEHBAZ-MD Bot</title>
        <style>
          body { font-family: Arial; background: linear-gradient(135deg, #121212 0%, #1f1f1f 100%); text-align: center; padding: 50px; color: #00ffcc; }
          .container { background: rgba(255,255,255,0.05); padding: 30px; border-radius: 10px; border: 1px solid #00ffcc; box-shadow: 0 0 15px rgba(0, 255, 204, 0.2); }
          h1 { font-size: 3em; color: #00ffcc; text-shadow: 0 0 10px rgba(0, 255, 204, 0.5); }
      ...
      <body>
        <div class="container">
          ...
          <p style="color: #ffffff;">Type .menu in WhatsApp to see commands</p>
          <p style="color: #aaaaaa;">Owner: ${config.OWNER_NAME || 'Shehbaz—Dev'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/shehbaz.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://files.catbox.moe/16i1l7.jpg'; // Default permanent image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 SHEHBAZ-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ SHEHBAZ-MD Bot connected to WhatsApp!");
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://files.catbox.moe/16i1l7.jpg');
